...
class task_queue:
	...
	def get_find(find_str):
		...
		request_df['is_latest'] = (
				request_df.groupby('name')['date_time'].transform('max') == request_df['date_time']).astype(int)

		data = []
		for i in request_df.index.tolist():
			row = request_df.loc[i]
			s_data = str(row['data']).replace('\'', '\\\"')
			if find_str in s_data:
				data.append(json.loads(s_data))
